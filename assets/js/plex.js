var PLEX = {
	...
	generate_item_content: function() {
		...
		if(PLEX.current_item.num_seasons && PLEX.current_item.num_seasons>0) {
			...
			$("#popup-container").on("click", "#popup_seasons_seasons li", function(){
			...
			$("#popup-container").on("click", "#popup_seasons_episodes li", function(){
	...
	run: function() {
		...
		if(!PLEX.data_loaded) {
			$.get("media/data.js", function(data){
				eval(data); // unpack
		...
		$(PLEX._genre_list).on("click", "#genre_show_all", function(){
		...
		$(PLEX._genre_list).on("click", "#genre_hide_all", function(){
		...
		$(PLEX._director_list).on("click", "#director_show_all", function(){
		...
		$(PLEX._director_list).on("click", "#director_hide_all", function(){
			PLEX.show_all_directors = false;
			$(".director_hidden").hide();
			$("#director_hide_all").hide();
		...
		$("#popup-container").on("click","#popup-footer span",  function(){
			PLEX.display_item($(this).attr("data-item"));
		});
